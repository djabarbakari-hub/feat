...
document.addEventListener("click", async (e) => {
  ...
  if (quizBtn) {
    ...
    state.quizStep += 1;
    if (state.bmiConsent === false && QUIZ_STEPS[state.quizStep]?.type === "optional") {
      state.quizStep += 1;
    }
    persistState();
    render();
  ...
  // [COMMENTAIRE] Gestionnaire d'action pour la suppression de compte client par un administrateur.
  // Permet à l'admin de purger l'intégralité des données Firestore de l'utilisateur visé.

...
export default function LoginPage() {
  const { signIn, signInGoogle, resetPassword, configured } = useAuth();
  ...
  const [info, setInfo] = useState(() => {
    ...
    return new URLSearchParams(window.location.search).get("created")
      ? "Compte créé. Vérifie ta boîte mail puis connecte-toi."
      : "";
  });
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setError(err.message || readableAuthError(err));
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <main className="screen screen--auth">
      ...
      <h1 className="h1">Content de te revoir.</h1>
      <p className="muted">Connecte-toi pour reprendre ta séance du jour.</p>
      ...
      <div className="auth-stack">
        ...
        >
          <GoogleIcon />
          {pending ? "Connexion Google…" : "Continuer avec Google"}
        </button>
        ...
        <button
          ...
          onClick={async () => {
            if (!email) {
              setError("Saisis d'abord ton e-mail.");
              return;
            }
            try {
      ...
      <p className="muted" style={{ marginTop: "auto" }}>
        ...
        <Link href="/signup" style={{ color: "var(--ember)", fontWeight: 800 }}>
          Créer un compte
        </Link>
      </p>
    </main>
